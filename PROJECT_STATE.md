...
per-module pacing. Per-module pacing gets its own short, real diagnostic at
the top of each module instead (`docs/teaching-style-prompt.md`'s "Probe,
then route") — this intake isn't a substitute for that, the two operate at
...
plausibly already-known) — but don't use them to skip a module's own
diagnostic probe. Don't default to assuming an experienced-engineer
fast-track without asking either the intake or the per-module probe — the old
version of this section did exactly that, and it was true for one specific
person's repo, not a safe default for whoever pulled this template.
